...
from maskrcnn_benchmark.data import make_data_loader, make_data_infer_loader
...
def main():
    ...
    if cfg.DATASETS.INFER_BBOX:
        if cfg.GLOBAL_SETTING.DATASET_CHOICE == 'VG':
            _ = checkpointer.load(cfg.MODEL.PRETRAINED_DETECTOR_CKPT_VG, with_optim=False)
        elif cfg.GLOBAL_SETTING.DATASET_CHOICE == 'OIV6':
            _ = checkpointer.load(cfg.MODEL.PRETRAINED_DETECTOR_CKPT_OIV6, with_optim=False)
        elif cfg.GLOBAL_SETTING.DATASET_CHOICE == 'GQA_200':
            _ = checkpointer.load(cfg.MODEL.PRETRAINED_DETECTOR_CKPT_GQA, with_optim=False)
        else:
            exit('wrong checkpointer name!')
    else:
