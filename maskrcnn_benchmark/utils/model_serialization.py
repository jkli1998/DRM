...
def align_and_update_state_dicts(model_state_dict, loaded_state_dict, load_mapping):
    ...
    for idx_new, idx_old in enumerate(idxs.tolist()):
        ...
        if ((not key.startswith('module.'))  and key != key_old) or (key.startswith('module.') and key[7:] != key_old):
            ...
            match = operator.eq(tuple(loaded_state_dict[key_old].shape), tuple(model_state_dict[key].shape))
            logger.info("LOADED! {}, {} load from {}, pretrain: {}, new: {}".format(
                match, key, key_old, tuple(loaded_state_dict[key_old].shape), tuple(model_state_dict[key].shape)
            ))
