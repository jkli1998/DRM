...
class RelationSampling(object):
    ...
    def half_copy(self, proposals, targets):
        assert len(proposals) == len(targets)
        half_length = len(proposals) // 2
        v1_proposals, v2_proposals = proposals[: half_length], proposals[half_length:]
        v1_targets, v2_targets = targets[: half_length], targets[half_length:]
        _, v1_labels, v1_idx_pairs, v1_sym_binarys = self.gtbox_relsample(v1_proposals, v1_targets)

        v1_labels.extend(copy.deepcopy(v1_labels))
        v1_idx_pairs.extend(copy.deepcopy(v1_idx_pairs))
        v1_sym_binarys.extend(copy.deepcopy(v1_sym_binarys))
        return proposals, v1_labels, v1_idx_pairs, v1_sym_binarys
    ...
    def half_detect_relsample(self, proposals, targets):
        half_length = len(proposals) // 2
        v1_proposals, v2_proposals = proposals[: half_length], proposals[half_length:]
        v1_targets, v2_targets = targets[: half_length], targets[half_length:]
        _, v1_labels, v1_idx_pairs, v1_sym_binarys = self.detect_relsample(v1_proposals, v1_targets)
        v1_labels.extend(copy.deepcopy(v1_labels))
        v1_idx_pairs.extend(copy.deepcopy(v1_idx_pairs))
        v1_sym_binarys.extend(copy.deepcopy(v1_sym_binarys))
        return proposals, v1_labels, v1_idx_pairs, v1_sym_binarys
