...
from maskrcnn_benchmark.modeling.roi_heads.relation_head.utils_co_attention import Self_Attention_Encoder, \
    Cross_Attention_Encoder, Entity2PredicateCrossAttentionEncoder, Predicate2EntityCrossAttentionEncoder
...
class SHA_Encoder(nn.Module):
    ...
    def forward(self, visual_feats, text_feats, num_objs):
        ...
        for enc_layer in self.cross_encoder:
...
class Entity2PredicateCell(nn.Module):
    def __init__(self, config, hidden_dim=None):
        super(Entity2PredicateCell, self).__init__()
        self.cfg = config
        if hidden_dim is None:
            self.hidden_dim = self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
        else:
        ...
        self.k_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.KEY_DIM
        self.v_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.VAL_DIM
        self.entity2prd = Entity2PredicateCrossAttentionEncoder(self.num_head, self.k_dim, self.v_dim,
                                                                self.hidden_dim, self.inner_dim, self.dropout_rate)


    def forward(self, predicate_feat, entity_feats, rel_pairs, num_objs, num_edges):
        assert num_objs is not None
        outp = self.entity2prd(predicate_feat, entity_feats, rel_pairs, num_objs, num_edges)
        return outp


class Predicate2EntityCell(nn.Module):
    def __init__(self, config, hidden_dim=None):
        super(Predicate2EntityCell, self).__init__()
        self.cfg = config
        if hidden_dim is None:
            self.hidden_dim = self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
        else:
        ...
        self.k_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.KEY_DIM
        self.v_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.VAL_DIM
        self.prd2entity = Predicate2EntityCrossAttentionEncoder(self.num_head, self.k_dim, self.v_dim,
                                                                self.hidden_dim, self.inner_dim, self.dropout_rate)

    def forward(self, entity_feats, predicate_feat, rel_pairs, num_objs, num_rels):
        outp = self.prd2entity(entity_feats, predicate_feat, rel_pairs, num_objs, num_rels)
        return outp


class PredicateEntityAttention(nn.Module):
    def __init__(self, config):
        super(PredicateEntityAttention, self).__init__()
        self.cfg = config
        self.SA_Cell_prd = Self_Attention_Cell(config)
        self.SA_Cell_ent = Self_Attention_Cell(config)
        self.CA_Cell_prd = Entity2PredicateCell(config)
        self.CA_Cell_ent = Predicate2EntityCell(config)

    def forward(self, predicate_feat, entity_feat, rel_pairs, num_objs, num_rels):
        psa = self.SA_Cell_prd(predicate_feat, num_objs=num_rels)
        pca = self.CA_Cell_prd(predicate_feat, entity_feat, rel_pairs, num_objs, num_rels)
        esa = self.SA_Cell_ent(entity_feat, num_objs=num_objs)
        eca = self.CA_Cell_ent(entity_feat, predicate_feat, rel_pairs, num_objs, num_rels)
        predicate_output = psa + pca
        entity_output = esa + eca
        return predicate_output, entity_output


class PredicateEntityEncoder(nn.Module):
    def __init__(self, config, n_layers):
        super(PredicateEntityEncoder, self).__init__()
        self.cfg = config
        self.dropout_rate = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.DROPOUT_RATE
        self.num_head = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.NUM_HEAD
        self.inner_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.INNER_DIM
        ...
        self.v_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.VAL_DIM
        self.cross_encoder = nn.ModuleList([
            PredicateEntityAttention(config)
            for _ in range(n_layers)])

    def forward(self, predicate_feat, entity_feat, rel_pairs, num_objs, num_rels):
        predicate_output = predicate_feat
        entity_output = entity_feat
        for enc_layer in self.cross_encoder:
            predicate_output, entity_output = enc_layer(predicate_output, entity_output, rel_pairs, num_objs, num_rels)

        # predicate_output = predicate_output + entity_output
        return predicate_output, entity_output
...
class SHA_Dual_Context(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        super().__init__()
        self.cfg = config
        # setting parameters
        ...
        self.lin_edge_visual_prd = nn.Linear(self.hidden_dim + self.in_channels, self.hidden_dim)
        self.lin_edge_textual_prd = nn.Linear(self.embed_dim, self.hidden_dim)

        self.lin_edge_visual_tpt = nn.Linear(self.hidden_dim + self.in_channels, self.hidden_dim)
        self.lin_edge_textual_tpt = nn.Linear(self.embed_dim, self.hidden_dim)

        self.out_obj = nn.Linear(self.hidden_dim, self.num_obj_cls)

        self.context_obj = SHA_Encoder(config, self.obj_layer)
        self.context_edge_prd = SHA_Encoder(config, self.edge_layer)
    ...
    def forward(self, roi_features, proposals, logger=None):
        ...
        # prd context
        edge_ctx_prd = self.edge_prd(roi_features, obj_feats, obj_preds, num_objs)
        # tpt context
        edge_ctx_tpt = self.edge_tpt(roi_features, obj_feats, obj_preds, num_objs)

        return obj_dists, obj_preds, (edge_ctx_prd, edge_ctx_tpt)

    def edge_prd(self, roi_features, obj_feats, obj_preds, num_objs):
        edge_pre_rep_vis = cat((roi_features, obj_feats), dim=-1)
        edge_pre_rep_txt = self.obj_embed_prd(obj_preds)

        edge_pre_rep_vis = self.lin_edge_visual_prd(edge_pre_rep_vis)
        edge_pre_rep_txt = self.lin_edge_textual_prd(edge_pre_rep_txt)
        edge_ctx_vis, _ = self.context_edge_prd(edge_pre_rep_vis, edge_pre_rep_txt, num_objs)
        return edge_ctx_vis

    def edge_tpt(self, roi_features, obj_feats, obj_preds, num_objs):
        edge_pre_rep_vis = cat((roi_features, obj_feats), dim=-1)
        ...
        edge_ctx_vis, _ = self.context_edge_tpt(edge_pre_rep_vis, edge_pre_rep_txt, num_objs)
        return edge_ctx_vis
