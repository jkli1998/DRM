...
def remove_non_alpha(strings):
    result = [''.join(ch for ch in s if ch.isalpha()) for s in strings]
    return result
...
def obj_edge_vectors(names, wv_dir, wv_type='glove.6B', wv_dim=300):
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            re_lw_token = remove_non_alpha(lw_token)
            print("{} -> {} ".format(token, " ".join(re_lw_token)))
            tag = True
            for item_token in re_lw_token:
                wv_index = wv_dict.get(item_token, None)
                if wv_index is not None:
                    if tag:
                        vectors[i] = wv_arr[wv_index]
                        tag = False
