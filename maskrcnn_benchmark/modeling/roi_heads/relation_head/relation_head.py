...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
            ...
            with torch.no_grad():
                if self.cfg.MODEL.STAGE == "stage1":
                    if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                        result = self.samp_processor.half_copy(proposals, targets)
                    else:
                        result = self.samp_processor.half_detect_relsample(proposals, targets)
                elif self.cfg.MODEL.STAGE == "stage2":
                    if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                        result = self.samp_processor.gtbox_relsample(proposals, targets)
                    else:
                        result = self.samp_processor.detect_relsample(proposals, targets)
                proposals, rel_labels, rel_pair_idxs, rel_binarys = result
        else:
            if self.cfg.MODEL.INFER_TRAIN:
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    result = self.samp_processor.gtbox_relsample(proposals, targets)
                else:
                    result = self.samp_processor.detect_relsample(proposals, targets)
        ...
        if self.training or self.cfg.MODEL.INFER_TRAIN:
            for x, y in zip(proposals, targets):
                x.add_field("image_index", y.get_field("image_index"))
                if y.has_field("aug_mask"):
                    x.add_field("aug_mask", y.get_field("aug_mask"))
        ...
        refine_logits, relation_logits, add_losses = self.predictor(
            proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, union_features
        )
