...
class FPN2MLPFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels, half_out=False, cat_all_levels=False):
        ...
        self.fc7 = make_fc(representation_size, out_dim, use_gn)
        dimension = cfg.MODEL.ROI_RELATION_HEAD.UNION_COMPRESS_DIM
        if cfg.MODEL.ROI_RELATION_HEAD.UNION_COMPRESS:
            self.fc8 = make_fc(representation_size, dimension, use_gn)
        self.out_channels = out_dim
        self.copy_tag = True
    ...
    def forward_without_pool(self, x):
        x = x.view(x.size(0), -1)
        x = F.relu(self.fc6(x))
        if self.cfg.MODEL.ROI_RELATION_HEAD.UNION_COMPRESS:
            x = F.relu(self.fc8(x))
        else:
            x = F.relu(self.fc7(x))
        return x
