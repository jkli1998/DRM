...
from .lr_scheduler import WarmupMultiStepLR, WarmupReduceLROnPlateau, WarmupCosStepLR


def make_optimizer(cfg, model, logger, slow_heads=None, slow_ratio=5.0, rl_factor=1.0, fast_heads=None, fast_ratio=10.0):
    params = []
...
def make_lr_scheduler(cfg, optimizer, logger=None):
    ...
    if cfg.SOLVER.SCHEDULE.TYPE == "WarmupCosStepLR":
        return WarmupCosStepLR(
            optimizer,
            warmup_factor=cfg.SOLVER.WARMUP_FACTOR,
            warmup_iters=cfg.SOLVER.WARMUP_ITERS,
            cosine_factor=cfg.SOLVER.COSINE_FACTOR,
            max_iteration=cfg.SOLVER.MAX_ITER,
        )
    else:
