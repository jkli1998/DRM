...
class WarmupCosStepLR(torch.optim.lr_scheduler._LRScheduler):
    def __init__(
        self,
        optimizer,
        warmup_factor=1.0 / 3,
        warmup_iters=500,
        cosine_factor=7.0 / 16,
        last_epoch=-1,
        max_iteration=50000,
    ):
        self.warmup_factor = warmup_factor
        self.warmup_iters = warmup_iters
        self.c_f = cosine_factor
        self.max_iteration = max_iteration
        assert self.c_f <= 0.5
        super(WarmupCosStepLR, self).__init__(optimizer, last_epoch)
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_iters:
            ...
            return [
                base_lr
                * math.cos(self.c_f * math.pi * (self.last_epoch-self.warmup_iters) / (self.max_iteration-self.warmup_iters))
                for base_lr in self.base_lrs
            ]
