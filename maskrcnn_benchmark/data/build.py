...
from .collate_batch import BatchCollator, BBoxAugCollator, ContrastiveBatchCollator
...
def get_dataset_statistics(cfg):
    ...
    result = {
        ...
        'pred_matrix': statistics[0]['pred_matrix'],
        'data_length': statistics[0]['data_length'],
        'down_rate': statistics[0]['down_rate'],
        'generate_rate': statistics[0]['generate_rate'],
        'repeat_dict': statistics[0]['repeat_dict'],
    }
...
def _compute_aspect_ratios(dataset):
    ...
    for i in range(len(dataset)):
        if dataset.repeat_dict is not None:
...
def make_batch_data_sampler(
    dataset, sampler, aspect_grouping, images_per_batch, num_iters=None, start_iter=0, mode='test',
):
    drop_uneven = False if mode in {'test', 'val'} else True
    if aspect_grouping:
        ...
        # --change --
        batch_sampler = samplers.GroupedBatchSampler(
            sampler, group_ids, images_per_batch, drop_uneven=drop_uneven
        )
        ...
        # --change --
        batch_sampler = torch.utils.data.sampler.BatchSampler(
            sampler, images_per_batch, drop_last=drop_uneven
        )
...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0, is_label=None):
    ...
    for dataset in datasets:
        ...
        batch_sampler = make_batch_data_sampler(
            dataset, sampler, aspect_grouping, images_per_gpu, num_iters, start_iter, mode
        )
        if mode == 'train' and cfg.MODEL.STAGE == 'stage1':
            collator = BBoxAugCollator() if not is_train and cfg.TEST.BBOX_AUG.ENABLED else \
                ContrastiveBatchCollator(cfg, cfg.DATALOADER.SIZE_DIVISIBILITY)
        else:
            collator = BBoxAugCollator() if not is_train and cfg.TEST.BBOX_AUG.ENABLED else \
                BatchCollator(cfg, cfg.DATALOADER.SIZE_DIVISIBILITY, mode=mode)
...
def make_data_infer_loader(cfg, mode='train', is_distributed=False, start_iter=0, is_label=None):
    assert mode in {'train', 'val', 'test'}
    num_gpus = get_world_size()
    ...
    # If bbox aug is enabled in testing, simply set transforms to None and we will apply transforms later
    transforms = None if cfg.TEST.BBOX_AUG.ENABLED else build_transforms(cfg, False)
    datasets = build_dataset(cfg, dataset_list, transforms, DatasetCatalog, False)
    ...
    for dataset in datasets:
        ...
        sampler = make_data_sampler(dataset, shuffle, is_distributed)
        batch_sampler = make_batch_data_sampler(
            dataset, sampler, aspect_grouping, images_per_gpu, num_iters, start_iter, 'test'
        )

        collator = BBoxAugCollator() if cfg.TEST.BBOX_AUG.ENABLED else \
            BatchCollator(cfg, cfg.DATALOADER.SIZE_DIVISIBILITY, mode="None")
        num_workers = cfg.DATALOADER.NUM_WORKERS
        data_loader = torch.utils.data.DataLoader(
            dataset,
            num_workers=num_workers,
