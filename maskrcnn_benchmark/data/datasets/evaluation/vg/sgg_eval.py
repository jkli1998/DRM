...
class SGRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, cfg):
    ...
    def generate_print_string(self, mode):
        ...
        result_str += '\n'
        return result_str, self.container
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        f1_mask = rel_scores[:, 0] < 1 - 1e-10
        local_container['pred_f1_num'] = np.sum(f1_mask)
        ...
        pred_rels, pred_scores = rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, overall_scores)
        ...
        local_container['pred_triplets'] = pred_triplets
        ...
        prd_contain = predcls_rel_matcher(gt_rels, pred_rels, gt_classes, pred_classes,
                                          gt_triplet_boxes, pred_triplet_boxes, pred_triplets)
        self.container.append(prd_contain)
        return local_container


def predcls_rel_matcher(gt_rels, pred_rels, gt_classes, pred_classes,
                        gt_triplet_boxes, pred_triplet_boxes, pred_triplets):
    prd_contain = []
    gt_sub_id, gt_ob_id = gt_rels[:, 0], gt_rels[:, 1]
    pd_sub_id, pd_ob_id = pred_rels[:, 0], pred_rels[:, 1]
    gt_pair = np.column_stack((gt_classes[gt_sub_id], gt_classes[gt_ob_id]))
    pd_pair = np.column_stack((pred_classes[pd_sub_id], pred_classes[pd_ob_id]))
    class_unaware = _compute_pred_matches(pd_pair, gt_pair, pred_triplet_boxes, gt_triplet_boxes, 0.999, phrdet=False)
    for item in class_unaware:
        i_contain = []
        for x in item:
            i_contain.append(pred_triplets[x])
        prd_contain.append(i_contain)
    return prd_contain
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        filter_idx = rel_nms_ng(pred_boxes, pred_classes, nogc_pred_rels)
        nogc_pred_rels = nogc_pred_rels[filter_idx][:100]
        nogc_pred_scores = nogc_pred_scores[filter_idx][:100]
...
def rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, overall_scores):
...
def rel_nms_ng(pred_boxes, pred_classes, nogc_pred_rels):
    ious = bbox_overlaps(pred_boxes, pred_boxes)
    sub_ious = ious[nogc_pred_rels[:, 0]][:, nogc_pred_rels[:, 0]]
    obj_ious = ious[nogc_pred_rels[:, 1]][:, nogc_pred_rels[:, 1]]
    rel_ious = np.minimum(sub_ious, obj_ious)
    sub_labels = pred_classes[nogc_pred_rels[:, 0]]
    obj_labels = pred_classes[nogc_pred_rels[:, 1]]
    rel_labels = nogc_pred_rels[:, 2]

    is_overlap = (rel_ious >= 0.5) & (sub_labels[:, None] == sub_labels[None, :]) & (
            obj_labels[:, None] == obj_labels[None, :]) & (rel_labels[:, None] == rel_labels[None, :])

    record_overlap = np.zeros(nogc_pred_rels.shape[0], dtype=np.bool_)
    sparse_nogc_pred_ind = np.zeros(nogc_pred_rels.shape[0], dtype=np.bool_)

    for i in range(nogc_pred_rels.shape[0]):
        if record_overlap[i]:
            pass
        else:
            sparse_nogc_pred_ind[i] = True
            record_overlap[is_overlap[i]] = True
    return sparse_nogc_pred_ind
