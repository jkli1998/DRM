...
def do_vg_evaluation(
    ...
    zeroshot_triplet = torch.load(os.path.join(cfg.DATASETS.DATA_DIR, "vg/zeroshot_triplet_0_new_order.pytorch"), map_location=torch.device("cpu")).long().numpy()
    ...
    if "relations" in iou_types:
        ...
        eval_recall = SGRecall(result_dict, cfg)
        eval_recall.register_container(mode)
        evaluator['eval_recall'] = eval_recall

        # no graphical constraint
        eval_nog_recall = SGNoGraphConstraintRecall(result_dict, cfg)
        ...
        result_str += r_str
        torch.save(r_data, os.path.join(cfg.OUTPUT_DIR, 'recall_result.pytorch'))
